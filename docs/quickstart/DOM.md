...
* самозакрывающиеся html теги, как в данном примере `input` рекомендуем закрывать принудительно. 
...
В данном решении мы показали возможность ручного рендеринга `view` по событиям, шаблон как строку для `view`, передачу данных в конструктор и работу с событиями в DOM.

> обратите внимание на то что `events` определены функцией, это вызвано тем что мы изпользуем бабель плагин, который не совсем корректно оборачивает свойства экземпляра класса, плагин навигатор умеет с этим работать но в данном случае мы его ен использовали: `el: '#screen'`
