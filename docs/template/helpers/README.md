...
```
**Description:**
(*Helpers*) Задает имя функции, которую можно использовать непосредственно в шаблоне хелперов для рендеринга вложенного контента(transclude helpers).
...
Определяет имя объекта данных из родительского хелпера в шаблоне.
...
```
**Description:**
(*Helpers*) Задает имя объекта данных, которые передаються в функцию рендеренга контента и будут доступены в шаблоне, который использует данный хелпер (transclude helpers).
...
#### <u>options.MAP (Object):</u>
Определяет набор компонентов, хелперов.
...
не должен быть пустой елемент что бы небыло elementVoid, должно быть значение атрибута, в качестве значения может быть выражение или true или false
...
Шаблонах для хелперов вы так же можете использовать `ref` для того чтобы получить прямую ссылку на DOM элемент в javascript.
