RAD.views.SwipeAdapterView = RAD.Blanks.View.extend({
...
// example
RAD.models.MyCollection = (function () {
    "use strict";
    ...
    for (i = 0; i < PAGES; i += 1) {
        ...
        for (j = 0; j < CHANNELS; j += 1) {
            p.channels.push({name: alphabet[j / 2], data: getRandomArray()});
        }
        p.count = count;
        allCount += count;
        o.push(p);
...
RAD.views.InnerFirstWidget = RAD.views.SwipeAdapterView.extend({
    ...
    getData: function (position) {
        ...
        model.all = RAD.models.MyCollection.allCount;
        return model;
    },
